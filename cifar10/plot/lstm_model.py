...
df = pd.read_csv('/home/zmou1/scratchenalisn1/ziyao/l2d-cog/cifar10H/results/mixed_lstm_curve_per_step_T200_finetune.csv')
...
os.makedirs("figures/curves", exist_ok=True)
...
plt.ylabel("Accuracy")
plt.title("LSTM L2D Model Accuracy on CIFAR-10H Over Trial Index")
plt.legend(loc='lower right', fontsize=8)
plt.ylim(0.5, 1.0)
plt.tight_layout()
plt.savefig("figures/mixed_curves/LSTM_model_accuracy_over_trial_index.png", dpi=200)
# plt.show()  # 无显示环境可注释掉
...
# 注意：散点也用 trial_index 做 x 轴，避免错位
...
plt.xlabel("Trial Index")
plt.ylabel("Accuracy")
plt.title("LSTM L2D Model Accuracy on CIFAR-10H Over Trial Index (Smoothed)")
plt.ylim(0.5, 1.0)
plt.grid(True, alpha=0.25, linestyle="--")
plt.legend(loc="lower right", fontsize=9)
plt.tight_layout()
plt.savefig("figures/mixed_curves/LSTM_model_accuracy_over_trial_index_smooth.png", dpi=200)
# plt.show()
...
plt.title("LSTM L2D Model Defer Rate on CIFAR-10H Over Trial Index")
plt.ylim(0.0, 1.0)
plt.grid(True, alpha=0.25, linestyle="--")
plt.legend(loc="lower right", fontsize=9)
plt.tight_layout()
plt.savefig("figures/mixed_curves/LSTM_model_defer_rate_over_trial_index_smooth.png", dpi=200)
