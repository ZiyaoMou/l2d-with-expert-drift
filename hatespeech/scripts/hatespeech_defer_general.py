...
base_model_root = f"model/general_models_new/alpha1.0_seq_len{seq_len}"
...
for seed in seed_list:
    ...
    model_path = os.path.join(model_root, f"best_model.pth")
    torch.save(best_model.state_dict(), model_path)

    rows = []
    for k in range(seq_len):
        sub_examples = [ex for ex in test_examples if int(vars(ex)['t']) == k]
        if len(sub_examples) == 0:
            continue
        sub_ds = data.Dataset(sub_examples, fields=fields_for_new)
        sub_it = data.BucketIterator(sub_ds, batch_size=BATCH_SIZE, sort=False, device=device)
        m = eval_metrics_dict(best_model, sub_it)
        rows.append({
            "timestep": k,
            "coverage": m["coverage"],
            "system_acc": m["system_acc"],
            "expert_acc": m["expert_acc"],
            "classifier_acc": m["classifier_acc"],
            "sample_count": m["sample_count"],
        })

    df = (pd.DataFrame(rows)
            .sort_values("timestep")[["timestep","coverage","system_acc","expert_acc","classifier_acc","sample_count"]])
    csv_path = os.path.join(model_root, "metrics_test.csv")
